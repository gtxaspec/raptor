...
static void handle_setup_event(rwc_state_t *st, const struct usb_ctrlrequest *req)
{
	...
	if (type & USB_DIR_IN) {
		...
		case UVC_GET_CUR:
			...
			break;
		case UVC_GET_MIN:
			fill_streaming_control(&sc, RWC_FMT_MJPEG, RWC_FRAME_360P,
					       RWC_INTERVAL_15FPS);
			memcpy(resp.data, &sc, len);
			resp.length = len;
			break;
		case UVC_GET_MAX:
			fill_streaming_control(&sc, RWC_FMT_H264, RWC_FRAME_1080P,
					       RWC_INTERVAL_30FPS);
			memcpy(resp.data, &sc, len);
			resp.length = len;
			break;
		case UVC_GET_DEF:
			fill_streaming_control(&sc, RWC_FMT_MJPEG, RWC_FRAME_1080P,
					       RWC_INTERVAL_30FPS);
			memcpy(resp.data, &sc, len);
			resp.length = len;
			break;
...
static int start_streaming(rwc_state_t *st)
{
	...
	if (ioctl(st->uvc_fd, VIDIOC_S_FMT, &fmt) < 0)
		...
		RSS_DEBUG("V4L2 format: %ux%u sizeimage=%u", fmt.fmt.pix.width, fmt.fmt.pix.height,
	...
	for (i = 0; i < st->buf_count; i++) {
		...
		st->buffers[i].start = mmap(NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED,
...
static void feed_audio(rwc_state_t *st)
{
	...
	for (int i = 0; i < RWC_AUDIO_DRAIN_MAX; i++) {
		...
		int ret = rss_ring_read(st->audio_ring, &st->audio_seq, st->audio_buf,
					st->audio_buf_size, &alen, &ameta);
		if (ret == RSS_EOVERFLOW && st->audio_seq > 0) {
			st->audio_seq--;
			ret = rss_ring_read(st->audio_ring, &st->audio_seq, st->audio_buf,
					    st->audio_buf_size, &alen, &ameta);
		}
		if (ret != 0 || alen == 0)
			break;
...
static void check_ring_idle(rss_ring_t **ring, uint64_t *last_ws, int *idle_count, const char *name)
{
	if (!*ring)
		return;
...
static int uvc_open(rwc_state_t *st)
{
	...
	for (int i = 0; i < (int)(sizeof(events) / sizeof(events[0])); i++) {
		...
		if (ioctl(st->uvc_fd, VIDIOC_SUBSCRIBE_EVENT, &sub) < 0) {
			RSS_ERROR("subscribe event %u: %s", events[i], strerror(errno));
			close(st->uvc_fd);
			st->uvc_fd = -1;
			return -1;
		}
	...
	fill_streaming_control(&st->probe, RWC_FMT_MJPEG, RWC_FRAME_1080P, RWC_INTERVAL_30FPS);
	st->commit = st->probe;
...
static int ctrl_handler(const char *cmd_json, char *resp, int resp_size, void *ud)
{
	...
	char res_str[16];
	snprintf(res_str, sizeof(res_str), "%ux%u",
	...
	cJSON_AddStringToObject(r, "resolution", res_str);
	cJSON_AddNumberToObject(r, "fps", st->cur_interval ? 10000000 / st->cur_interval : 0);
	cJSON_AddBoolToObject(r, "audio", st->audio_enabled);
...
int main(int argc, char **argv)
{
	...
	if (st.buf_count < 2)
		st.buf_count = 2;
	if (st.buf_count > RWC_MAX_BUFFERS)
