...
void ric_poll_exposure(ric_state_t *st)
{
	...
	/*
	 * Zero means "this backend cannot answer for that field", which is
	 * the exposure contract raptor-hal documents (hal_isp.c, both
	 * backends) rather than a convention invented here. Honouring it is
	 * not optional for luma: a live sensor never reports a mean luma of
	 * exactly 0, and the day→night test is `ae_luma < night_luma`, so a
	 * zero taken as data is the darkest reading there is and pins the
	 * camera in night mode for good. SigmaStar Infinity6E did exactly
	 * that until it had an isp_get_exposure at all.
	 */
	...
	if (st->settings.trigger == RIC_TRIGGER_PHOTO) {
		...
		/* Every phase of the photo state machine is a comparison
		 * against ev; with no ev there is nothing to run. Fall back
		 * rather than idle, and only say so once -- changing the
		 * trigger is what makes it once. */
	...
	if (st->settings.trigger == RIC_TRIGGER_LUMA) {
		 ...
		 * platform with gain but no luma runs on gain alone instead
		 * of reading the absent luma as pitch darkness.
		 */
		...
		if (have_gain && st->night_gain_baseline > 0) {
			...
			/*
			 * No gain, so no baseline and no ratio: the only
			 * signal left is the inverse of the day→night test.
			 * Weaker than the ratio -- IR illumination lifts luma
			 * too -- but a platform that reports luma and not gain
			 * would otherwise never come back out of night, which
